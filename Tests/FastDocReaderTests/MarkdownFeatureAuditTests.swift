...
final class MarkdownFeatureAuditTests: XCTestCase {
    ...
    private func textTableBlocks(_ s: NSAttributedString) -> [NSTextTableBlock] {
        var blocks: [NSTextTableBlock] = []
        s.enumerateAttribute(.paragraphStyle, in: NSRange(location: 0, length: s.length)) { v, _, _ in
            guard let ps = v as? NSParagraphStyle else { return }
            for tb in ps.textBlocks { if let b = tb as? NSTextTableBlock { blocks.append(b) } }
        }
        return blocks
    }
    ...
    func testTableRealGrid() {
        XCTAssertFalse(textTableBlocks(render("| A | B |\n|---|---|\n| 1 | 2 |")).isEmpty)
    }
